...
func (f *fileReader) createTempFile() error {
	...
	if f.file == nil {
		...
		if f.xorSecret != "" {
			osFileXor, _ := vfs.Create(mos, "filexor")
			_, _ = io.Copy(osFileXor, XorTransformer(osFile, f.xorSecret))
			f.file = osFileXor
...
func createTempFile(f *zip.File) (vfs.File, error) {
	reader, err := f.Open()
	if err != nil {
		return nil, err
	}
	...
	tempFile, err := vfs.Create(mos, "zipfs")
	if err != nil {
		return nil, err
	}
